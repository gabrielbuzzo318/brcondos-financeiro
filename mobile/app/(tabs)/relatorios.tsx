...
export default function Relatorios(){
  const items=[['Fluxo de Caixa','Entradas e saídas por período'],['DRE','Demonstrativo de Resultado'],['Inadimplência','Lista e evolução das cobranças'],['Contas a Pagar','Por fornecedor e categoria'],['Contas a Receber','Por condomínio e período'],['Extrato do Condomínio','Histórico completo']];
  return <ScrollView style={s.page} contentContainerStyle={s.content}><Text style={s.title}>Relatórios</Text><Text style={s.sub}>Os relatórios mais úteis da BRCondos no celular.</Text><View style={s.card}>{items.map(([a,b])=><View key={a} style={s.row}><View style={s.icon}><Text>▥</Text></View><View style={{flex:1}}><Text style={s.name}>{a}</Text><Text style={s.desc}>{b}</Text></View><Text style={s.chev}>›</Text></View>)}</View></ScrollView>
}
