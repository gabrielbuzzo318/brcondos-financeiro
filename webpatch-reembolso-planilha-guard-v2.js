(function(){
  ...
  const REQUIRED=new Set(CANONICAL.map(x=>x.sourceKey));
  const boot=Date.now();
  let restoring=false;
  ...
  function hasAllCanonical(rows){
    const keys=new Set((Array.isArray(rows)?rows:[]).map(x=>String(x?.sourceKey||'')));
    for(const k of REQUIRED)if(!keys.has(k))return false;
    return true;
  }

  function replaceWithCanonical(){
    if(restoring)return false;
    const rows=getRows();
    if(hasAllCanonical(rows))return false;
    restoring=true;
    const next=CANONICAL.map(x=>({...x}));
    ...
    try{localStorage.setItem('brcondos_reimbursements_planilha_v2','ok')}catch(_){ }
  ...
  if(typeof original==='function'){
    window.renderReimbursements=function(){
      if(Date.now()-boot<15000)replaceWithCanonical();
      return original.apply(this,arguments);
    };
  }

  [0,50,150,400,900,1800,3500,7000,12000].forEach(ms=>setTimeout(()=>{
    const changed=replaceWithCanonical();
    if(changed){try{if(typeof renderAll==='function')renderAll()}catch(_){ }}
  },ms));
})();